#!/usr/bin/env python3
"""Fetch agent transcripts from GitHub repos and build JSON for the replay app.

Groups individual agent transcript files by sessionId so the replay app
shows one combined timeline per team session with all agents interleaved.

Usage:
    python build-data.py                          # fetch from default sources
    python build-data.py owner/repo               # fetch from a specific repo
    python build-data.py owner/repo .claude/transcripts  # custom path
    python build-data.py owner/repo .claude/transcripts abc123  # pinned ref
"""

import re
import json
import sys
import os
import urllib.request
import urllib.error
import base64
from collections import defaultdict
from pathlib import Path
from concurrent.futures import ThreadPoolExecutor, as_completed

OUTPUT_FILE = Path(__file__).parent / "data" / "transcripts.json"

# ─── Default transcript sources ──────────────────────────────────
# Each source is a GitHub owner/repo + path + optional git ref.
# Add entries here to pull transcripts from additional repos.
SOURCES = [
    {
        "owner": "Randroids-Dojo",
        "repo": "UnrealFrog",
        "path": ".claude/transcripts",
        "ref": "39db5cc3ed7076b46cc0660d58e70aa1f0304556",
    },
]

AGENT_META = {
    "engine-architect": {"initials": "EA", "color": "#10B981", "label": "Engine Architect", "role": "C++ Systems"},
    "game-designer":    {"initials": "GD", "color": "#F59E0B", "label": "Game Designer",    "role": "Mechanics & Feel"},
    "xp-coach":         {"initials": "XP", "color": "#3B82F6", "label": "XP Coach",         "role": "Process Lead"},
    "devops-engineer":  {"initials": "DE", "color": "#8B5CF6", "label": "DevOps Engineer",  "role": "Build & CI"},
    "qa-lead":          {"initials": "QA", "color": "#06B6D4", "label": "QA Lead",           "role": "Testing & Feel"},
    "level-designer":   {"initials": "LD", "color": "#EF4444", "label": "Level Designer",   "role": "Spatial Design"},
    "art-director":     {"initials": "AD", "color": "#EC4899", "label": "Art Director",     "role": "Visual Style"},
    "sound-engineer":   {"initials": "SE", "color": "#F97316", "label": "Sound Engineer",   "role": "Audio Design"},
    "team-lead":        {"initials": "TL", "color": "#6366F1", "label": "Team Lead",        "role": "Coordinator"},
    "user":             {"initials": "U",  "color": "#94A3B8", "label": "Stakeholder",      "role": "Direction"},
}

MAX_WORKERS = 10


# ─── GitHub helpers ───────────────────────────────────────────────

def github_headers():
    """Return headers for GitHub API requests, using GITHUB_TOKEN if available."""
    headers = {"Accept": "application/vnd.github.v3+json", "User-Agent": "GameTape-build"}
    token = os.environ.get("GITHUB_TOKEN")
    if token:
        headers["Authorization"] = f"token {token}"
    return headers


def github_api_get(url):
    """GET a GitHub API URL and return parsed JSON."""
    req = urllib.request.Request(url, headers=github_headers())
    with urllib.request.urlopen(req, timeout=30) as resp:
        return json.loads(resp.read().decode())


def github_raw_get(owner, repo, ref, filepath):
    """Download a raw file from GitHub."""
    url = f"https://raw.githubusercontent.com/{owner}/{repo}/{ref}/{filepath}"
    req = urllib.request.Request(url, headers={"User-Agent": "GameTape-build"})
    with urllib.request.urlopen(req, timeout=30) as resp:
        return resp.read().decode()


def list_transcript_files(owner, repo, path, ref):
    """List .md files in a GitHub directory."""
    url = f"https://api.github.com/repos/{owner}/{repo}/contents/{path}"
    if ref:
        url += f"?ref={ref}"
    items = github_api_get(url)
    return sorted(
        [item["path"] for item in items if item["name"].endswith(".md")],
    )


def fetch_transcript(owner, repo, ref, filepath):
    """Download and parse a single transcript file from GitHub."""
    content = github_raw_get(owner, repo, ref, filepath)
    name = Path(filepath).stem
    return name, content


# ─── Transcript parsing ──────────────────────────────────────────

def parse_metadata(content):
    meta = {}
    for key, pattern in [
        ("date",      r'\| Date \| (\d{4}-\d{2}-\d{2}) \|'),
        ("sessionId", r'\| Session ID \| `([^`]+)` \|'),
        ("model",     r'\| Model \| ([^\|]+) \|'),
        ("duration",  r'\| Duration \| ([^\|]+) \|'),
        ("tokens",    r'\| Tokens \| ([^\|]+) \|'),
    ]:
        m = re.search(pattern, content)
        if m:
            meta[key] = m.group(1).strip()
    return meta


def parse_messages(content):
    messages = []
    sections = re.split(r'^(## (?:User|Assistant) \(\d{2}:\d{2} UTC\))', content, flags=re.MULTILINE)
    last_teammate = None

    for i in range(1, len(sections), 2):
        header = sections[i]
        body = sections[i + 1] if i + 1 < len(sections) else ""

        hm = re.match(r'## (User|Assistant) \((\d{2}:\d{2}) UTC\)', header)
        if not hm:
            continue

        role, time = hm.group(1), hm.group(2)

        tm = re.search(
            r'<teammate-message\s+teammate_id="([^"]+)"(?:\s+color="([^"]*)")?(?:\s+summary="([^"]*)")?\s*>',
            body
        )

        if role == "User" and tm:
            sender = tm.group(1)
            summary = tm.group(3) or ""

            cm = re.search(r'<teammate-message[^>]*>(.*?)</teammate-message>', body, re.DOTALL)
            msg_content = cm.group(1).strip() if cm else body.strip()

            receiver = "team-lead"
            to_match = re.search(r'\[to\s+([a-z][\w-]*)\]', summary, re.IGNORECASE)
            if to_match:
                receiver = to_match.group(1)

            msg_type = "idle" if '"type": "idle_notification"' in msg_content else "teammate"

            messages.append({
                "time": time, "from": sender, "to": receiver,
                "content": msg_content[:800], "summary": summary, "type": msg_type
            })
            last_teammate = sender

        elif role == "User" and not tm:
            msg_content = body.strip()
            if msg_content and len(msg_content) > 10:
                messages.append({
                    "time": time, "from": "user", "to": "team-lead",
                    "content": msg_content[:800], "summary": "", "type": "user"
                })
                last_teammate = None

        elif role == "Assistant":
            msg_content = body.strip()
            lines = [l for l in msg_content.split('\n')
                     if l.strip() and not l.strip().startswith('- **Used')]
            if lines:
                receiver = last_teammate or "all"
                msg_type = "send" if '**Used SendMessage**' in body else "response"
                clean = '\n'.join(lines)
                messages.append({
                    "time": time, "from": "team-lead", "to": receiver,
                    "content": clean[:800], "summary": "", "type": msg_type
                })

    return messages


def parse_transcript_content(name, content):
    metadata = parse_metadata(content)
    messages = parse_messages(content)

    parts = name.split('-', 4)
    title = parts[4].replace('-', ' ').title() if len(parts) > 4 else name

    participants = set()
    for msg in messages:
        participants.add(msg['from'])
        if msg['to'] != 'all':
            participants.add(msg['to'])

    return {
        "id": name, "title": title, "metadata": metadata,
        "messages": messages, "participants": sorted(participants),
        "messageCount": len(messages)
    }


def merge_session(transcripts):
    """Merge multiple transcript files from the same session into one combined timeline."""
    primary = max(transcripts, key=lambda t: t['messageCount'])
    meta = primary['metadata'].copy()

    all_participants = set()
    for t in transcripts:
        all_participants.update(t['participants'])

    merged = []
    for file_idx, t in enumerate(transcripts):
        for msg_idx, msg in enumerate(t['messages']):
            merged.append((msg['time'], file_idx, msg_idx, msg))

    merged.sort(key=lambda x: (x[0], x[1], x[2]))
    messages = [m[3] for m in merged]

    session_id = meta.get('sessionId', primary['id'])
    return {
        "id": session_id,
        "title": primary['title'],
        "metadata": meta,
        "messages": messages,
        "participants": sorted(all_participants),
        "messageCount": len(messages),
        "fileCount": len(transcripts),
    }


# ─── Source fetching ──────────────────────────────────────────────

def fetch_source(source):
    """Fetch and parse all transcripts from a single GitHub source."""
    owner = source["owner"]
    repo = source["repo"]
    path = source.get("path", ".claude/transcripts")
    ref = source.get("ref", "main")

    label = f"{owner}/{repo}"
    print(f"Listing transcripts in {label} ({path})...")

    try:
        files = list_transcript_files(owner, repo, path, ref)
    except urllib.error.HTTPError as e:
        print(f"  Error listing {label}: {e}", file=sys.stderr)
        return []

    print(f"  Found {len(files)} transcript files, downloading...")

    transcripts = []
    errors = 0

    def download(filepath):
        return fetch_transcript(owner, repo, ref, filepath)

    with ThreadPoolExecutor(max_workers=MAX_WORKERS) as pool:
        futures = {pool.submit(download, f): f for f in files}
        for future in as_completed(futures):
            filepath = futures[future]
            try:
                name, content = future.result()
                t = parse_transcript_content(name, content)
                has_teammate = any(m['type'] in ('teammate', 'idle') for m in t['messages'])
                if has_teammate and t['messages']:
                    transcripts.append(t)
            except Exception as e:
                errors += 1
                print(f"  Error fetching {Path(filepath).name}: {e}", file=sys.stderr)

    print(f"  Parsed {len(transcripts)} transcripts with agent messages" +
          (f" ({errors} errors)" if errors else ""))
    return transcripts


# ─── Main ─────────────────────────────────────────────────────────

def main():
    # Allow overriding sources via CLI: owner/repo [path] [ref]
    sources = SOURCES
    if len(sys.argv) >= 2:
        repo_arg = sys.argv[1]
        if "/" in repo_arg:
            owner, repo = repo_arg.split("/", 1)
            path = sys.argv[2] if len(sys.argv) >= 3 else ".claude/transcripts"
            ref = sys.argv[3] if len(sys.argv) >= 4 else "main"
            sources = [{"owner": owner, "repo": repo, "path": path, "ref": ref}]
        else:
            print(f"Usage: {sys.argv[0]} [owner/repo] [path] [ref]")
            sys.exit(1)

    all_transcripts = []
    for source in sources:
        all_transcripts.extend(fetch_source(source))

    if not all_transcripts:
        print("No transcripts with agent messages found.")
        sys.exit(1)

    # Group by sessionId
    by_session = defaultdict(list)
    for t in all_transcripts:
        sid = t['metadata'].get('sessionId', t['id'])
        by_session[sid].append(t)

    # Merge each session's transcripts into one combined timeline
    sessions = []
    for sid, group in by_session.items():
        sessions.append(merge_session(group))

    # Sort newest first by date then session id
    sessions.sort(key=lambda s: (s['metadata'].get('date', ''), s['id']), reverse=True)

    OUTPUT_FILE.parent.mkdir(parents=True, exist_ok=True)
    with open(OUTPUT_FILE, 'w') as f:
        json.dump({"agents": AGENT_META, "transcripts": sessions}, f)

    total_msgs = sum(s['messageCount'] for s in sessions)
    total_files = sum(s.get('fileCount', 1) for s in sessions)
    print(f"\nMerged {total_files} transcripts into {len(sessions)} sessions")
    print(f"Total messages: {total_msgs}")
    print(f"Output: {OUTPUT_FILE} ({OUTPUT_FILE.stat().st_size / 1024:.1f} KB)")


if __name__ == "__main__":
    main()
